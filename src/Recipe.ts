...
export class Recipe {
  ...
  #_encodeForCall(): Array<string | Array<any>> {
    ...
    const taskStruct = [this.name, ...transposed];
    return [taskStruct];
  }
  ...
  encodeForDsProxyCall(): Array<string> {
    const executeTaskAbi: any = RecipeAbi.find(
      ({ name }: { name: string }) => name === 'executeRecipe',
    );
    ...
    return [
      ...
      AbiCoder.encodeFunctionCall(executeTaskAbi, encoded), /// HEEREEE
    ];
  }
  ...
  _validateParamMappings() {
    this.actions.forEach((action: Action, i: number) => {
      action._getArgumentMapping().forEach((source, j) => {
        if (source) {
          console.log(
            `${this.actions[i].name} takes argument #${j + 1} from ${
              this.actions[source - 1].name
            } (action #${source})`,
          );
        }
      });
   ...
   */
  async getAssetsToApprove(): Promise<
  Array<
  | { owner: string; asset: string; specialApproveLabel?: string }
  | {
    owner: string;
    nft: EthAddress;
    tokenId: string;
  ...
  >
  > {
    const uniqueAssetOwnerPairs: Array<
    | {
      owner: string;
      asset: string;
      specialApproveLabel?: string;
    ...
    > = [];
    const assetOwnerPairs = await Promise.all(
      this.actions.map((a) => a.getAssetsToApprove()),
    );
    for (const pairsPerAction of assetOwnerPairs) {
      for (const pair of pairsPerAction) {
        const isNft = !pair.asset;
        if (
          !uniqueAssetOwnerPairs.find(
            (_pair) => _pair.owner === pair.owner
              && (isNft
                ? _pair.tokenId === pair.tokenId
                : _pair.asset === pair.asset),
          )
        ) {
          if (isNft) {
    ...
    return uniqueAssetOwnerPairs.filter(
      ({ asset }) => !compare(asset, getAssetInfo('ETH').address),
    );
  }
