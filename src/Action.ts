...
export class Action {
    ...
    return this.mappableArgs.map((arg) => {
      if (new RegExp(/\$\d+/).test(arg)) {
        if (Array.isArray(arg)) {
          throw TypeError(
            "Input can't be mapped to arrays (tuples/structs). Specify `mappableArgs` array in constructor.",
          );
        }
  ...
  encodeForDsProxyCall(): Array<string | string[]> {
    const executeActionDirectAbi: any = ActionAbi.find(
      ({ name }: { name: string }) => name === 'executeActionDirect',
    )!;
    return [
  ...
  encodeForRecipe(): Array<string | string[] | number[]> {
    return [
      ...
      this.getAddress(), // targetAddresses
